 ...
 */
package org.hnau.commons.app.model.color.dynamic.temperature

import org.hnau.commons.app.model.color.dynamic.hct.Hct
import org.hnau.commons.app.model.color.dynamic.utils.ColorUtils
...
class TemperatureCache(
    ...
    fun getAnalogousColors(
        ...
        val allColors: MutableList<Hct> = arrayListOf()
        ...
        val answers: MutableList<Hct> = arrayListOf()
        answers.add(input)
    ...
    private val hctsByHue: List<Hct>
        get() {
            ...
            val hcts: MutableList<Hct> = arrayListOf()
            var hue = 0.0
            while (hue <= 360.0) {
    ...
    private val hctsByTemp: List<Hct>
        get() {
            ...
            val hcts: MutableList<Hct> = hctsByHue.toMutableList()
            hcts.add(input)
    ...
    private val tempsByHct: Map<Hct, Double>
        get() {
            ...
            val allHcts: MutableList<Hct> = hctsByHue.toMutableList()
            allHcts.add(input)
            val temperaturesByHct: MutableMap<Hct, Double> = hashMapOf()
            for (hct in allHcts) {
    ...
    companion object {
        ...
        fun rawTemperature(color: Hct): Double {
            ...
            val hue = MathUtils.sanitizeDegreesDouble(atan2(lab[2], lab[1]) * 180 / PI)
            val chroma = hypot(lab[1], lab[2])
            return -0.5 +
                0.02 * chroma.pow(1.07) * cos(MathUtils.sanitizeDegreesDouble(hue - 50.0) * PI / 180)
        }
